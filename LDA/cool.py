...
def LDACoolKernel(K,alpha,beta,nwords):
...
#MAIN CHANGE: P_Z deleted!!!!!!!!!!!
def LDACOOLGPU(data, wz, dz, z, K,it,alpha,beta,PATH=""):
	...
	z = np.array(z,dtype=np.float32,order='C')
	...
	#** Create Kernel
	func = LDACoolKernel(K,alpha,beta,nwords)
	...
	for i in range(it):
		...
		if PATH!="":
			if (i+1)%10==0:
				np.save(PATH+"wz_lda"+"_".join(map(str,[algo,K,alpha,beta,(i+1)])),wz)
				np.save(PATH+"dz_lda"+"_".join(map(str,[algo,K,alpha,beta,(i+1)])),dz)
